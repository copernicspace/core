...
describe('[test/offers/instant/float-royalties-offer.test] Instant offer with royalties', () => {
	...
	it('should have success status of buy tx', async () => {
		...
		expect(txr.status).to.be.eq(TX_RECEIPT_STATUS.SUCCESS)
		expect(await cargoContract.balanceOf(creator.address, rootId), 'wrong creator balance of sold asset').to.be.eq(
			totalSupply.sub(buyAmount)
		)
		expect(await cargoContract.balanceOf(userA.address, rootId), 'wrong balance of buyer after tx buy').to.be.eq(
			buyAmount
		)
	...
	it('userA should be able to create resell offer with royalties included', async () => {
		...
		const userABalance = await cargoContract.balanceOf(userA.address, rootId)

		const txr = await instantOffer
			.connect(userA)
			.sell(cargoContract.address, rootId, userABalance, minBuyAmount, resellPrice, erc20Mock.address)
			.then(tx => tx.wait())
		expect(txr.status).to.be.eq(TX_RECEIPT_STATUS.SUCCESS)
