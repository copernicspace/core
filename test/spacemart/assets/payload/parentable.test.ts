...
describe('[asset/payload/parentable.test] `PayloadAsset`: parentable fixture test suite', () => {
	...
	it('disallows non-creators from creating child asset', async () => {
		await expect(
			payloadAsset
				.connect(receiver)
				.createChild(50, childID, 'revert.test.com', receiver.address, 'TEST_CHILD_CID', 0)
	...
	it('reverts if insufficient balance', async () => {
		await expect(
			payloadAsset
				.connect(creator)
				.createChild(50, childID, 'revert-insufficient.test.com', creator.address, 'TEST_CHILD_CID', 0)
	...
	it('create grandChild', async () => {
		const tx = await payloadAsset
			.connect(creator)
			.createChild(100, '0', ' test', creator.address, 'TEST_CHILD_CID', 0)
		const newChildId = await tx.wait().then(txr => getAssetID(txr))
		expect(newChildId).to.be.eq(2)

		const gtx = await payloadAsset
			.connect(creator)
			.createChild(100, newChildId, 'nameGrandChild', creator.address, '`TEST_CHILD_CID`', 0)
