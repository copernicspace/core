...
describe('[many-users-add-liquidity.test.ts]',
	() => {
		...
		it('checks all users have zero liquidity initially', async () => {
			expect(await spacePool.connect(userA).getMyLiquidity()).to.be.eq('0')
			expect(await spacePool.connect(userB).getMyLiquidity()).to.be.eq('0')
			expect(await spacePool.connect(userC).getMyLiquidity()).to.be.eq('0')
			expect(await spacePool.connect(userD).getMyLiquidity()).to.be.eq('0')
		})
